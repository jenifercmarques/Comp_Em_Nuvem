...
class EventCreate(BaseModel):
    """Payload para criar evento manualmente em ``POST /events``."""
    ...
    task_id: int | None = Field(
        default=None,
        description="ID da tarefa relacionada (quando aplicável).",
        examples=[42],
    )
    message: str = Field(
        ...,
        ...
        examples=["Tarefa criada via Swagger."],
    )


class EventRead(BaseModel):
    """Evento retornado por ``GET /events`` e ``POST /events``."""

    id: str = Field(..., examples=["a7f9f3b3-5cb4-4f1f-8c7e-6d2ee98e7b31"])
    event_type: str = Field(..., examples=["task.updated"])
    task_id: int | None = Field(default=None, examples=[42])
    message: str = Field(..., examples=["Tarefa 42 atualizada para done."])
    created_at: str = Field(
        ...,
        description="Data/hora em ISO 8601 (UTC).",
        examples=["2026-07-12T03:45:00Z"],
    )
